...
VALUES ('Я', 'Явка', 'Рабочий день'),
       ...
       ('В', 'Выходной', 'Выходной день')
ON CONFLICT (code) DO NOTHING;

INSERT INTO payment_types (code, name, category, description)
VALUES ('ОКЛ', 'Оклад', 'accrual', 'Основной оклад по отработанному времени'),
       ('ОТП', 'Отпуск', 'accrual', 'Оплата дней трудового отпуска'),
       ('БОЛ', 'Больничный', 'accrual', 'Оплата дней отсутствия по листку нетрудоспособности'),
       ('ПХД', 'Премия за ПХД', 'accrual', 'Премия за производственно-хозяйственную деятельность'),
       ('ПСС', 'Премия за сниж. С/С', 'accrual', 'Премия за снижение себестоимости продукции'),
       ('СТАЖ', 'Надбавка за стаж', 'accrual', 'Доплата за продолжительность работы'),
       ('ИТР', 'Премия ИТР', 'accrual', 'Премия инженерно-техническим работникам'),
       ('ПРЕД', 'Премия по предприятию', 'accrual', 'Общая премия по предприятию'),
       ('ПН', 'Подоходный налог', 'deduction', 'Налог на доходы физических лиц'),
       ('ФСЗН', 'Взнос в ФСЗН', 'deduction', 'Отчисления в фонд социальной защиты'),
       ('АЛ', 'Алименты', 'deduction', 'Удержание по исполнительному листу'),
       ('ПВ', 'Профсоюзный взнос', 'deduction', 'Членский взнос в профсоюз')
ON CONFLICT (code) DO NOTHING;
